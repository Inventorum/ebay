...
from inventorum_ebay.lib.core_api.models import CoreOrder
from inventorum_ebay.lib.core_api.tests.factories import CoreOrderFactory
from inventorum_ebay.apps.orders.core_orders_sync import CoreOrdersSync, CoreOrderSyncer
from inventorum_ebay.apps.orders.models import OrderModel
from inventorum_ebay.apps.orders.tasks import periodic_core_orders_sync_task
from inventorum_ebay.apps.orders.tests.factories import OrderModelFactory
from inventorum_ebay.apps.shipping.models import ShippingServiceModel
from inventorum_ebay.lib.celery import celery_test_case, get_anonymous_task_execution_context
from inventorum_ebay.lib.ebay.data.events import EbayEventType
...
class IntegrationTestPeriodicCoreOrdersSync(EbayAuthenticatedAPITestCase):
    ...
    @celery_test_case()
    def test_smoke(self):
        core_api_mock = self.patch("inventorum_ebay.apps.accounts.models.EbayAccountModel.core_api",
                                   new_callable=PropertyMock(spec_set=UserScopedCoreAPIClient))

        ebay_events_publish_mock = self.patch("inventorum_ebay.lib.ebay.events.EbayInboundEvents.publish")
        ebay_orders_complete_sale_mock = self.patch("inventorum_ebay.lib.ebay.orders.EbayOrders.complete_sale")
...
class UnitTestCoreOrdersSync(UnitTestCase):
    ...
    def create_mocks(self):
        ...
        core_order_syncer = "inventorum_ebay.apps.orders.core_orders_sync.CoreOrderSyncer.__call__"
...
class UnitTestCoreOrderSyncer(UnitTestCase):
    ...
    def setup_mocks(self):
        self.schedule_ebay_order_status_update_mock = \
            self.patch("inventorum_ebay.apps.orders.tasks.schedule_ebay_order_status_update")

        self.schedule_click_and_collect_status_update_with_event = \
            self.patch("inventorum_ebay.apps.orders.tasks.schedule_click_and_collect_status_update_with_event")
