...
from inventorum_ebay.apps.categories.tasks import initialize_periodic_ebay_categories_sync_task
...
class TestPeriodicEbayCategoriesSync(UnitTestCase):
    ...
    def test_invokes_correct_service_methods(self):
        categories_syncer_mock = self.patch("inventorum_ebay.apps.categories.tasks.EbayCategoriesScraper.fetch_all")
        features_syncer_mock = self.patch("inventorum_ebay.apps.categories.tasks.EbayFeaturesScraper.fetch_all")
        specifics_syncer_mock = self.patch("inventorum_ebay.apps.categories.tasks.EbaySpecificsScraper.fetch_all")
